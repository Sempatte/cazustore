		...
		<>
			<div className="container pt-5">
				<div className="row">
					...
					{
						Products.map((item) => (
							<Item  key={item.id} stock={item.stock} onAdd={addItems} name={item.name} stars_count={item.stars} initial_price={item.initial_price} final_price={item.final_price} img1={item.image1} img2={item.image2} hot={item.hot} />
						))
					}
